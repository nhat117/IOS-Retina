...
struct FormComponent: View {
    let text: String
    let scrWidth: CGFloat
    @Binding var textInput: String
    var body : some View {
        Group{
            TextField(text, text: $textInput).padding(.leading)
                .textCase(.lowercase)
                .autocapitalization(.none)
            
            Rectangle()
                .frame(width: scrWidth - 50, height: 1)
        }
    }
}
...
struct FormComponent_Previews: PreviewProvider {
    static var previews: some View {
        FormComponent(text: "Hi", scrWidth: 300.0, textInput: .constant("Some Input"))
    }
}
